...
**Version hiện tại:** `2026.08.28-r147`
...
### 6.7 KPI 7 — PAVOI KPI (tỷ lệ PAVOI chưa được đánh giá) — r142

Card `KPI 7 — PAVOI KPI (not-assessed rate)` trên trang `#page-kpi`, ngay dưới card *KPI QC — ATA Compare*.
...
- **⭐ Công thức (bản sửa r144 — điểm dễ làm sai nhất): tử số và mẫu số nằm trên HAI TẬP KHÁC NHAU, đây là chủ ý.**
  ```
  TONG  = OPEN + CLOSED                             (mẫu số: MỌI PAVOI đã phát hành)
  OPEN  = OPEN_DA_DANH_GIA + OPEN_CHUA_DANH_GIA     (chỉ trong tập Open)
  KPI%  = OPEN_CHUA_DANH_GIA ÷ TONG × 100
  ```
  **PAVOI đã Closed KHÔNG BAO GIỜ vào tử số, nhưng LUÔN nằm trong mẫu số.** Càng thấp càng tốt, `0%` = không còn việc đang mở nào chưa đánh giá. `Total = 0` → `—` (không chia 0, không `NaN`).
  ⚠️ r142 làm sai: đếm **mọi** PAVOI không có task vào tử số kể cả đã đóng → đo trên data thật 2026 ra **15.9%** thay vì **7.9%** (5 PAVOI đã đóng không có task bị tính nhầm).
- **Cột bảng phản chiếu đúng phân hoạch:** `Open · assessed` + `Open · not assessed` + `Closed` + (`open not loaded`) = `Total PAVOI`.
- **Phạm vi mẫu số:** **mọi trạng thái** PAVOI (Open, Overdue, đã đóng), lọc theo **Năm** `#kpi7Year` (`raised_year`, mặc định năm hiện tại, dùng lại `initYearMulti`/`ymHas`).
...
- **`kpi7Stats()`** là nguồn DUY NHẤT cho render, modal và self-check → ba nơi không thể lệch nhau.
- **Self-check (§14 r142):** mỗi dòng `assessed + notAssessed === total`; `Σ total 5 dòng con === total MQA`. Chỉ chạy khi `kpi7Done.size > 0` (trước đó mọi số đều 0 → kiểm "đúng" một cách vô nghĩa — đúng bài học r141).
