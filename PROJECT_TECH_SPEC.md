...
**Version hiện tại:** `2026.06.12-r108-i1`
...
Đo nội bộ phát hiện sớm authority: **`Ratio[ATA] = (CMR-CAR + QC Spot Check finding tháng N−1) ÷ (ECAR finding tháng N)`**, hiển thị **số thập phân** (`(num/den).toFixed(2)`, KHÔNG phải %). (Thay ratio cũ "ECAR/CMR under-detection".)
...
- **Chart:** grouped bar ngang, 2 dataset Internal (xanh) vs ECAR (đỏ), top 15 ATA theo tổng. **Bảng:** dòng **TOTAL** (r108-i1: **chỉ cộng phần CÓ ATA** = `allATAFull.reduce`, khớp đúng tổng các dòng per-ATA — KHÔNG gồm No-ATA để tránh "Internal=70 nhưng rows cộng ~41") → per-ATA → dòng **No-ATA** (finding thiếu ATA, gom `Other`, để riêng, không tính ratio — KHÔNG giấu mẫu số). Ô count rỗng hiển thị `0` (không phải `—`).
...
- **Versioning (`APP_REV = YYYY.MM.DD-rNN[-iM]`):** phân biệt **nâng rev** vs **đánh issue** để rev không bị đẩy lên quá cao do mỗi lần vá lỗi.
  ...
  - *Ví dụ:* r107 = tính năng QCS detail (nâng rev). 2 hotfix sau (TDZ trắng data; timeout `report_field`) là **bug fix → Issue 1 & Issue 2 của r107** (`r107-i1`, `r107-i2`), KHÔNG nâng lên r108/r109. r108 = tính năng KPI ATA mới (nâng rev); 3 sửa hiển thị/tính TOTAL sau đó = **Issue 1 (`r108-i1`)**, KHÔNG nâng rev. Hiện **r108-i1**.
