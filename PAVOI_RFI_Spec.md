...
>
> **r129 thêm (Owner + Section filter):** bảng PAVOI thêm cột **Owner** (`r.owner_name` = `userMap[owner_id].full_name`, đã có sẵn từ `loadData`) đặt **ngay sau Report No**, sortable qua `pavSort('owner_name')`. Thêm dropdown **Section** (`#pavoiSectF`, class `.sel`). `owner_name` cũng thêm vào haystack search.
>
> **r130 (sửa hướng filter — QUAN TRỌNG):** r129 lọc Section theo trường **Department** là **sai**. Đúng ra 4 nhóm `MQA/AMO - … Section` là **User Group** dùng làm **OWNER** của PAVOI (tính năng Galileo dựng sẵn; owner đang chuyển từ cá nhân → nhóm, probe **28/461 PAVOI đã có owner = group**). r130: (a) `loadData` nạp `dwanalytics_user_group` (group_id→name) merge vào `userMap` để `owner_name` hiện đúng tên nhóm khi owner là group; (b) dropdown value = **đúng 4 tên nhóm**, predicate `if(sect && r.owner_name!==sect) return false;`. Department quay lại thuần cột hiển thị. Cột **Owner** giữ nguyên — chỉ việc phản ánh đúng owner thật (cá nhân giờ / nhóm sau).
