...
13. **Corrective actions của MỌI form (r169, probe 26/09/2026).** `loadEisDetail` đọc nhóm CORRECTIVE ACTIONS của mọi report trong `allData` (không riêng EIS): `report_field` theo 6 `field_name` (`Corrective Action(s)` · `Corrective Action Classification` · `Department` · `Target date` · `Date completed` · `Verification of implementation (VOI)`, ~41.800 dòng toàn hệ thống, ~4s), post-filter report_id, gom theo `section_id`. **Một section chỉ là CA khi có field `Corrective Action(s)`** — `Department` / `Target date` / `Date completed` là tên dùng chung; luật cũ `action||target||dept` trên mọi form đếm nhầm phần đầu report thành CA (MCAR 395, PAVOI 246, AMO ECAR 133…). Kết quả (mọi năm): F-088 **166** CA / 55 report · EIS **62** (hai luật trùng nhau → EIS không đổi) · MQA Event Investigation Report **15** · AMO ECAR **20**. `MQA Investigation Report` và `Meeting Minutes - MOM` có field này nhưng không có report trong `allData`. Trạng thái CA = `semCalc(reportClosed ? 'Closed' : 'Open', Date completed, Target date)`; report đã đóng mà CA thiếu Date completed → `Closed` (không chấm đúng/trễ hạn). **Không có trường gia hạn theo CA** trên Coruson → chưa tách được Extension ở cấp CA.
