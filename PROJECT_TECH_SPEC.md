...
| Rev | Nội dung |
|---|---|
| r132 | **`loadData()` nạp song song — bỏ ~17s xếp hàng vô ích.** Sáu request đầu của `loadData` (report summary · users · user groups · report workflow · custom fields · audit summary) **độc lập** — không cái nào dùng kết quả cái kia làm `$filter` — nhưng trước đây chạy `await` nối tiếp. Đo trên proxy 19/08/2026: TTFB của Galileo là **2,0–4,1s mỗi request** trong khi truyền chỉ **0,07–0,18s**, tổng tuần tự **~22s** cho ~38 MB JSON (5,1 MB qua dây nhờ brotli). Chuyển sang `Promise.all` → còn **~5s** (= request chậm nhất, custom fields 4,3s). Mỗi job dựng map ngay trong `onRows` để nhả mảng thô sớm (workflow 22 MB, custom fields 10,7 MB) thay vì giữ cả sáu tới khi barrier xong. Giữ nguyên phân tầng lỗi: chỉ summary là `critical` (hỏng → `showLoadError`), năm nguồn còn lại nuốt lỗi + trả `[]` đúng như `try/catch` từng bước cũ. Users áp trước user-groups **sau** barrier nên thứ tự merge không phụ thuộc độ trễ mạng. Overlay đổi từ nhãn cố định `[1/6]…[6/6]` sang bộ đếm `parDone` tăng theo request về đích. **Không đổi dữ liệu đầu ra** — các `forEach` dựng `wfMap`/`wfAllMap`/`fMap`/dedup summary giữ nguyên từng dòng. Đã đo và LOẠI hai hướng khác: lọc `report_workflow` theo `report_title in (37 title QA AMO)` chỉ giảm 119.041→90.664 dòng (24%) mà TTFB tăng 3,7s→5,5s; lọc theo `report_created_date ge 2021-10-01` chỉ giảm còn 109.013. |
