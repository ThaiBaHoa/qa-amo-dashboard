...
**Version hiện tại:** `2026.08.25-r143`
...
- **Công thức:** `KPI% = Not assessed ÷ Total × 100`. **Càng thấp càng tốt**, `0%` = đã đánh giá hết. `Total = 0` → hiển thị `—` (không chia 0, không `NaN`).
- **Phạm vi:** **mọi trạng thái** PAVOI (Open, Overdue, đã đóng), lọc theo **Năm** `#kpi7Year` (`raised_year`, mặc định năm hiện tại, dùng lại `initYearMulti`/`ymHas`).
